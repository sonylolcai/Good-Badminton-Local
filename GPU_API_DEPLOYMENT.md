...
| 已验证公网映射 | `http://xn-g.suanjiayun.com:52028`（部署时实测 HTTP 200） |
| 当前平台端口口径 | 用户确认后续公网入口为 `8080`；必须以健康检查复验后才可替代上行已验证的 `52028` 记录 |
...
将公网端口（当前计划使用 `8080/TCP`）的入站来源限制为业务服务器的公网 IP。不要把 `.gpu-api.env` 上传、提交或发送到聊天中。每次修改端口映射后，从业务服务器复验：

```bash
curl -fsS http://xn-g.suanjiayun.com:8080/api/v1/health
```

返回 `status: ok` 说明该公网端口确实转发到容器内 API。若平台配置为 `公网 8080 → 容器 8001`，程序无需改端口；只有平台明确要求容器自身监听 `8080` 时，才修改 `.gpu-api.env` 中的 `PORT` 并重启 API。
...
nohup python3 -m uvicorn api.app:app --host 0.0.0.0 --port "${PORT:-8001}" > gpu-api.log 2>&1 &
echo $! > .gpu-api.pid
sleep 3
curl -fsS "http://127.0.0.1:${PORT:-8001}/api/v1/health"
```
