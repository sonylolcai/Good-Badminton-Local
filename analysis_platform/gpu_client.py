    ...
    try:
        ...
        for artifact in segmenter.iter_input(str(source), overwrite=True):
            if cancel_cb is not None and cancel_cb():
                client.cancel()
                raise AnalysisCancelled("远端 GPU 分片任务已中断。")
        ...
        terminal = _wait_for_stream_terminal(
            client,
...
def _wait_for_stream_terminal(client, *, poll_interval_seconds, timeout_seconds, cancel_cb=None):
    terminal_states = {"finalized", "partial", "failed", "cancelled", "interrupted_needs_rebuild"}
    ...
    raise TimeoutError("stream session did not reach a terminal state before timeout")
...
def cancel_remote_two_second_stream(analysis_session_id, gpu_base_url=None, *, sport_id="badminton"):
    """Request cancellation for one known stream session."""
    session_id = str(analysis_session_id or "").strip()
    if not re.fullmatch(r"[A-Za-z0-9_-]{1,128}", session_id):
        raise RemoteAnalysisError("流会话 ID 格式无效")
    ...
    if not config["api_key"]:
        raise RemoteAnalysisError(f"GOOD_{sport_id.upper()}_GPU_API_KEY is not configured in the WebUI process")
    return _json_request(config, f"/api/v1/stream-sessions/{session_id}", method="DELETE")
...
def _materialize_stream_webui_result(client, config, target, terminal):
    ...
    try:
        if sport_id == "tennis":
            ...
            from analysis_platform.stream_speed_summary import summarize_stream_player_speeds
