"""Bounded player-report handoff owned by the business service.
...
def generate_performance_report(output_dir, metadata_path, spatial_summary_path):
    ...
    movement_metrics = _read_json(report_dir / "player_movement_metrics_v1.json")
    body_profiles = _read_json(report_dir / "player_body_profiles_v1.json")
...
def build_report_evidence(metadata, spatial_summary, *, movement_metrics=None, body_profiles=None):
    """Build compact, traceable inputs without manufacturing ability scores."""
    metadata = metadata or {}
    spatial_summary = spatial_summary or {}
    movement_metrics = movement_metrics or {}
    body_profiles = body_profiles or {}
    movement_by_track = {
        str(item.get("track_id")): item
        for item in movement_metrics.get("players") or []
        if isinstance(item, dict) and item.get("track_id")
    }
    profile_by_track = {
        str(item.get("track_id")): item
        for item in body_profiles.get("profiles") or []
        if isinstance(item, dict) and item.get("track_id")
    }
    players = []
        ...
        players.append(
            {
                ...
                "movement": {
                    ...
                    "post_match_metrics": (movement_by_track.get(str(item.get("track_id"))) or {}).get("movement"),
                    "energy_estimate": (movement_by_track.get(str(item.get("track_id"))) or {}).get("energy_estimate"),
                },
                "user_provided_body_profile": profile_by_track.get(str(item.get("track_id"))),
                "tracking_coverage": {
    ...
    return {
        ...
        "post_match_metric_policy": movement_metrics.get("measurement_policy"),
    }
