...
- 远端 GPU 仍以源码 ZIP、已安装 CUDA Python 运行时和持久化权重目录为部署边界；2026-09-16 追加本地 CPU Docker Compose 包装，消除 Windows 本机 Python/CompShare CLI 运行时依赖，不构建或部署远端 GPU Docker 镜像。
...
| AC-07 | 本地 Docker Compose 使用固定 Python 运行时启动同一多运动 API，health 同时报两项运动，权重目录映射到应用实际读取的 `/app/weights`。 |
