...
class EdgeClient:
    ...
    def heartbeat(self, capture_state: str, active_session_id: str | None, last_index: int | None) -> None:
...
def main() -> None:
    ...
    def heartbeats() -> None:
        while not stopped.wait(settings.heartbeat_seconds):
            try:
                client.heartbeat("capturing" if case_id else "idle", case_id, last_index)
            except requests.RequestException as error:
                print(f"heartbeat failed: {error}", flush=True)

    client.heartbeat("idle", None, None)
    ...
    try:
        ...
        if case_id is not None and last_index is not None:
            try:
                ...
                # The next operator action can inspect the still-active case;
                # never claim a successful close when the gateway was down.
